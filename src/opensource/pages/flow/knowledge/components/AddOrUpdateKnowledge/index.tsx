...
type AddOrUpdateKnowledgeForm = Pick<Knowledge.Detail, "name" | "description">

type AddOrUpdateKnowledgeProps = {
	knowledge?: Knowledge.Detail
	updateKnowledge?: (data: Knowledge.Detail) => void
	initKnowledgeList?: () => void
}

const useAddOrUpdateKnowledgeStyles = createStyles(({ css, isDarkMode, token }) => {
	return {
		iconEdit: css`
			padding: 4px;
			cursor: pointer;
...
function AddOrUpdateKnowledge({
	knowledge,
	updateKnowledge,
	initKnowledgeList,
}: AddOrUpdateKnowledgeProps) {
	const { t } = useTranslation()

	const { styles } = useAddOrUpdateKnowledgeStyles()

	const [open, { setTrue, setFalse }] = useBoolean(false)

	const [form] = useForm<AddOrUpdateKnowledgeForm>()
	...
	const handleOk = useMemoizedFn(async () => {
		try {
			...
			try {
				const data = await KnowledgeApi.saveKnowledge({
					id: knowledge?.id,
					name: res.name,
...
export default AddOrUpdateKnowledge
