...
export const generateCalendarApi = (fetch: HttpClient) => ({
	...
	updateCalendar(params: Calendar.SaveCalendarParams) {
		return fetch.put<{ id: string }>(
			genRequestUrl(RequestUrl.updateCalendar, { id: params.id! }),
			params,
		)
	},
	...
	getUserStatus(params: Calendar.GetUserStatusParams) {
		return fetch.post<Calendar.UserStatus[]>(genRequestUrl(RequestUrl.getUserStatus), params)
	},
